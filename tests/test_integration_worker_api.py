...
def test_worker_ingest_compute_and_api_query() -> None:
    ...
    with Session(engine) as s:
        s.merge(Ticker(symbol="FPT", name="FPT", exchange="HOSE", sector="Tech", industry="Software"))
        ...
        assert s.exec(select(IndicatorValue).where(IndicatorValue.symbol == "FPT")).first() is not None
    ...
    p = client.get("/prices", params={"symbol": "FPT", "timeframe": "1D", "start": "2025-01-01", "end": "2025-12-31", "limit": 10})
