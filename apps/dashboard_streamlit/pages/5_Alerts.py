...
expr = st.text_input("Expression", value="CROSSOVER(close, SMA(20)) AND volume > 1.5*AVG(volume, 20)")
...
if st.button("Create rule"):
    payload = {
        ...
        "timeframe": timeframe,
        "expression": expr,
        "symbols": [s.strip() for s in symbols.split(",") if s.strip()] if symbols.strip() else None,
    }
