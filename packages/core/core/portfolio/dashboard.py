...
def _load_portfolio_state(db: Session, portfolio_id: int) -> dict[str, Any]:
    ...
    port_panel = panel[[c for c in panel.columns if c in positions]].copy() if positions else pd.DataFrame(index=panel.index)
    ...
    risk_base = portfolio_risk_summary(port_val, bench) if not port_val.empty else {"volatility": 0.0, "max_drawdown": 0.0, "beta": 0.0, "var_95": 0.0}
...
def _capacity(positions: dict[str, Any], nav: float, tick_df: pd.DataFrame) -> dict[str, Any]:
    adtv_map = tick_df.set_index("symbol").get("adtv_20d", pd.Series(dtype=float)).to_dict() if not tick_df.empty else {}
    ...
    for sym, p in positions.items():
        ...
        if breached:
            flags.append({"symbol": sym, "type": "capacity_breach", "position_value": pos_value, "limit_value": limit_value})
    return {"by_symbol": by_symbol, "flags": flags}


def _constraints_snapshot(db: Session) -> dict[str, Any]:
    rows = db.exec(select(RebalanceConstraintReport).order_by(RebalanceConstraintReport.as_of_date.desc(), RebalanceConstraintReport.id.desc())).all()
    if not rows:
        return {"active": [], "violations_pre": [], "violations_post": [], "distance_metrics": {}}
    r = rows[0].report_json or {}
    return {
        "active": r.get("active_constraints", ["max_single", "max_sector", "min_cash", "liquidity cap", "cluster cap", "turnover cap"]),
...
def build_portfolio_dashboard(db: Session, portfolio_id: int) -> dict[str, Any]:
    ...
    tick_df = pd.DataFrame([t.model_dump() for t in tickers]) if tickers else pd.DataFrame(columns=["symbol", "sector", "beta", "adtv_20d"])
    ...
    style_exposure = {
        "large_cap": float(sum(1 for _, p in positions.items() if p.market_value >= nav * 0.03) / max(1, len(positions))),
        "mid_small": float(sum(1 for _, p in positions.items() if p.market_value < nav * 0.03) / max(1, len(positions))),
    }

    sec_map = tick_df.set_index("symbol").get("sector", pd.Series(dtype=object)).to_dict() if not tick_df.empty else {}
    cluster_map = tick_df.set_index("symbol").get("industry", pd.Series(dtype=object)).to_dict() if not tick_df.empty else {}
    ...
    names_contrib = [{"symbol": s, "contrib": float(abs(wt) * max(0.0, state["risk"]["vol"]))} for s, wt in w.sort_values(ascending=False).items()]
    sectors_contrib = [{"sector": str(k), "contrib": float(v)} for k, v in w.groupby(w.index.map(lambda x: sec_map.get(x, "Unknown"))).sum().sort_values(ascending=False).items()]
    clusters_contrib = [{"cluster": str(k), "contrib": float(v)} for k, v in w.groupby(w.index.map(lambda x: cluster_map.get(x, "Unknown"))).sum().sort_values(ascending=False).items()]
    ...
    return {
        ...
        "risk": state["risk"],
        "risk_contrib": {"names": names_contrib, "sectors": sectors_contrib, "clusters": clusters_contrib},
...
def build_rebalance_preview(db: Session, portfolio_id: int) -> dict[str, Any]:
    ...
    tickers = db.exec(select(Ticker)).all()
    tick_df = pd.DataFrame([t.model_dump() for t in tickers]) if tickers else pd.DataFrame(columns=["symbol"])
    ...
    pos = {
        h["symbol"]: Position(
            symbol=h["symbol"], quantity=h["quantity"], avg_cost=h["price"], market_price=h["price"], market_value=h["value"], unrealized_pnl=0.0
        )
    ...
    for s in reb.get("suggestions", []):
        ...
        adtv = float((tick_df.set_index("symbol").get("adtv_20d", pd.Series(dtype=float)).to_dict()).get(sym, 0.0) if not tick_df.empty else 0.0)
        ...
        trades.append(
            {
                ...
                "explain": {
                    "reason_key": key,
                    "inputs": {"weight": next((h["weight"] for h in dash["holdings"] if h["symbol"] == sym), 0.0)},
                },
            }
        )
    ...
    for t in trades:
        q = float(t["qty"])
        ac_schedule.append({"symbol": t["symbol"], "day1": q * 0.5, "day2": q * 0.3, "day3": q * 0.2})
...
def apply_scenario_shocks(dashboard: dict[str, Any], preview: dict[str, Any]) -> dict[str, Any]:
    ...
    largest_sector = next(iter(sorted(sector_weights.items(), key=lambda x: x[1], reverse=True)), (None, 0.0))[0]
    largest_sector_weight = float(sector_weights.get(largest_sector, 0.0)) if largest_sector else 0.0
    ...
    return {
        "S1_cost_x2": {"delta_expected_cost": s1_cost_total - float((preview.get("expected_costs") or {}).get("total", 0.0))},
        "S2_fill_x0_5": {"unfilled_notional": s2_unfilled, "fill_ratio": 0.5},
        "S3_market_down_5": {"nav_after": s3_nav, "delta_nav": s3_nav - nav},
        "S4_largest_sector_down_8": {"largest_sector": largest_sector, "nav_after": s4_nav, "delta_nav": s4_nav - nav},
    }
